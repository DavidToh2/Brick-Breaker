...
| | `currentLevel` | A `Level` object. <br/> `.ballSpeed`: stores the current (default) speed of the ball in-game. This value is incremented by 0.1 every 6 seconds.| `.loadLevel()`: Loads in the specified level. The layout data is stored as a string of characters, which is read directly by the `brick` constructor. <br/> `.updateHighScores()`: Updates high scores into the level datafile. |
